...
public class DungeonImpl implements IDungeon {
  ...
  private void updateEncounteredPitOrTheif(int nCurrR, int nCurrC) {
    if (null != this.gridTwoD && this.gridTwoD.isAPit(nCurrR, nCurrC)) {
      fallenIntoPit = true;
      isGameOver = true;
      isPlayWinner = false;
    }
    if (null != this.gridTwoD && this.gridTwoD.getRMonster(nCurrR, nCurrC) != null) {
      encounteredRMonster = true;
    } else {
      encounteredRMonster = false;
    }
    if (null != this.gridTwoD && this.gridTwoD.getThief(nCurrR, nCurrC) != null) {
  ...
  private void updatePlayerStateIfGameOver(int nCurrR, int nCurrC) {
    isGameOver = true;
    if (null != this.gridTwoD && null != this.gridTwoD.getMonster(nCurrR, nCurrC)) {
  ...
  public String getPlayersTreasures() {
    return null != this.habitant 
        ? this.habitant.getAvblTreasure() : "";
  }

  @Override
  public String getPlayersArrows() {
    return null != this.habitant 
        ? this.habitant.getAvblArrow() : "";
  }
  ...
  public String dungeonDescription() {
    return null != this.gridTwoD ? this.gridTwoD.dungeonDescription() : "";
  }
  ...
  public int fightMonster() {
    ...
    int nCurrR = newLocation / noOfColumns;
    int nCurrC = newLocation % noOfColumns;
    int r1 = (int)Math.floor(Math.random() * (2 ));
    int r2 = (int)Math.floor(Math.random() * (2 ));
    if (r1 == 1 && null != habitant) {
      habitant.updateStrength(); 
    } 
    if (r2 == 1 && null != gridTwoD.getRMonster(nCurrR, nCurrC)) {
    ...
    if (null != gridTwoD.getRMonster(nCurrR, nCurrC)
        && gridTwoD.getRMonster(nCurrR, nCurrC).getStrength() == 0) {
      gridTwoD.removeRMonster(nCurrR, nCurrC);
      encounteredRMonster = false;
    }

    if (null != habitant && habitant.getStrength() == 0) {
      isGameOver = true;
      isPlayWinner = false;
    }
