...
public class DungeonConsoleController implements DungeonController {
  ...
  public void playGame(IDungeon m) {
    ...
    try {
      out.append("Start point : " + m.getStartPoint() + "\n");
      out.append("End point : " + m.getEndPoint() + "\n");

      out.append(m.dungeonDescription());
      while (!m.isGameOver()) {
          ...
          if (row.equalsIgnoreCase("M") || row.equalsIgnoreCase("P") 
            ...
            if (row.equalsIgnoreCase("M")) {
              ...
              while (true) {
                ...
                row = scan.next();
                try {
                  int s = m.attack(row, distancerow);
                  if (s == 1) {
