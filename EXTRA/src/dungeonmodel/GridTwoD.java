...
class GridTwoD {
  ...
  public GridTwoD(int noOfRows, int noOfColumns, int interConnectivity,
    ...
    while (leastDistance < 5 || retMap.get(startPoint).size() == 2
      ...
      if (isRandom) {
        startPoint = new GenerateRandom().nextRandom(0, noOfRows * noOfColumns - 1);
        endPoint = new GenerateRandom().nextRandom(0, noOfRows * noOfColumns - 1);
      } else {
        startPoint = 1;
  ...
  /*public IHabitat[][] gettwoDimHabitat() {
    return this.twoDimHabitat;
  }*/
  ...
  public String removeTreasureOrArmor(String pickUpItem, int currR, int currC) {
    ...
    /*if (null != pickUpItem && !"".equals(pickUpItem)) {
      ...
      List<IWeapon> weaponList = twoDimHabitat[currR][currC].getAvblTreasure();
    }
    return name;
  ...
  public String getlocationDetails(int curr, int rowC, int colC) {
    ...
    //int rowC = cloc / twoDimHabitat[0].length;
    //int colC = cloc % twoDimHabitat[0].length;
  ...
  public int attack(int cloc, String dir, int distance) {
    ...
    int currStrength = 0;
    if (distance == 0 && null != twoDimHabitat[cloc / noOfColumns][cloc % noOfColumns].getMonster()) {
      currStrength = twoDimHabitat[cloc / noOfColumns][cloc % noOfColumns].getMonster().getStrength();
      if (currStrength > 0) {
  ...
  private int nextLocation(int cloc, String dir) {
    ...
    IHabitat habitat = twoDimHabitat[currR][currC];
    List<Integer> avblDir = habitat.getAvblDir(); 
    int location = nextR * twoDimHabitat.length + nextC;
